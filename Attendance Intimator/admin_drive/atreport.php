...
$period=$_GET['period'];
$year=$_GET['year'];
if($date=='' && $year=='' && $sem='' && $section='' && $period='' && $branch='')
{
...
{
$list=mysql_query("select id,name,year,sem,branch,section,period,subject,DATE_FORMAT(date,'%m/%d/%Y') date,atten from atten   where   date='$date' && year='$year' && sem='$sem' && section='$sec' && period='$period' && branch='$branch'");
}
